...
sigma = 75 #72 #72#1*72.0          # Surface tension (mN/m)
grav = 9.81e3#4*9.81e3        # Gravity (mm/s^2)
rneedle = 0.2075#2*0.3      # Needle radius (mm)
volume0 = 4.2#4.0    # Prescribed volume (mm^3)
deltarho = 1.e-3#1.26e-3 # Density difference (rho_top - rho_bottom) (g/mm^3)
...
niter = 100
for i = 1:niter
    global vars_sol, vars_num, shape_guess, rr, zz, s_uniform 
    volume = volume0/(niter+1-i)
    params_phys = ParamsPhys(sigma=sigma, grav=grav, rneedle=rneedle, volume0=volume, deltarho=deltarho)
    params_num = ParamsNum(N=N, nMaxIter=100, epsilon=1e-12)
    ...
    if i == 1 
        ...
        shape_guess.r = rr
        shape_guess.z = zz
        shape_guess.s = s_uniform
    end

    vars_sol, vars_num = gen_single_drop(params_phys, params_num; verbose=true, shape_guess = shape_guess)

    s_uniform = range(vars_num.s[1], stop=vars_num.s[end], length=Ngrid)
    rr_spline = Spline1D(vars_num.s, vars_sol.r)
    zz_spline = Spline1D(vars_num.s, vars_sol.z)
    
    rr = rr_spline(s_uniform)
    zz = zz_spline(s_uniform)
...
r = vars_sol.r
z = vars_sol.z
# rd = rand(Normal(0,0.05*rneedle), length(r)) .+ r
# zd = rand(Normal(0,0.05*rneedle), length(z)) .+ z
###################
### actual data ###
###################
...
# scatter!(figcomp, rd, zd; markershape=:circle, grid=true, xlim=[xmin, xmax], ylim=[ymin, ymax], aspect_ratio=:equal, label="data") # xlim=[-0.01, maximum(r)*1.1]
scatter!(figcomp, rd, zd; markershape=:circle, grid=true, aspect_ratio=:equal, label="data")
# plot!(figcomp, r, z; markershape=:circle, linewidth=2, label="model")
plot!(figcomp, rr, zz; markershape=:circle, linewidth=2, label="model")
